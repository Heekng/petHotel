...
public class Business {
	...
	public void customerLogin() { //고객로그인창
		...
		customerLoginView.addWindowListener(new WindowAdapter() {
			@Override
			public void windowClosing(WindowEvent e) {
				customerLoginView.dispose();
	...
	public void cusAppointment(int phoneNum) { //호텔 예약
		...
		cusAppointmentView.inputBt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!reEnterDate.isBefore(reExitDate)) {
					JOptionPane.showMessageDialog(null, "퇴실날짜는 입실날짜 이후로 선택해주세요.");
					return;
				}
	...
	public void lookUp() {
		...
		lookUpView.numBt.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if(!Pattern.matches("\\d{8}", lookUpView.numTf.getText())) {
			...
			changePetView.midButton[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(changePetView.changeRb.isSelected()) { //수정선택시
						...
						choiceChangeView.backBt.addActionListener(new ActionListener() {
							@Override
							public void actionPerformed(ActionEvent e) {
								choiceChangeView.dispose();
							}
						});
						choiceChangeView.addWindowListener(new WindowAdapter() {
							public void windowClosing(WindowEvent e) {
								choiceChangeView.dispose();
							};
						});
	...
	public void exitPetView() { //퇴실내역조회
		int size;
		ArrayList<String[]> data = new ArrayList<>();
		Iterator<Integer> keyItr = manager.getCustomerData().keySet().iterator();
		while(keyItr.hasNext()) {
			int key = keyItr.next();
			Customer cus = manager.getCustomerData().get(key);
		...
		AllExitPetView allExitPetView = new AllExitPetView(data);
		allExitPetView.addWindowListener(new WindowAdapter() {
			@Override
			public void windowClosing(WindowEvent e) {
				allExitPetView.dispose();
				managerMenuView.setVisible(true);
			}
		});
		allExitPetView.exitBt.addActionListener(new ActionListener() {
